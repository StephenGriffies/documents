...
\begin{itemize}
  ...
  \item [$\bullet$] The eddy-advection from eddies is related to
    Stokes' drift
...
\begin{frame}{Key paradigm: neutral diffusion + secondary circulation}
 ...
 \begin{itemize}
      \item[$\star$] Downscale turbulent cascade of tracer variance
        along along neutral directions leads to the notion of {\bf
          neutral diffusion}.
      \begin{itemize}
      \scriptsize
...
\begin{frame}{\textcite{Veronis1975}: serious problems with geopotential diffusion}
  ...
  \begin{itemize}
    ...
    \item[$\star$] Geopotential oriented diffusion is problematic
     ...
     \item[$\star$] Spurious dianeutral diffusivity scales like $A^{\mathsf{\scriptscriptstyle hordif}} \,
       |{\bm S}|^{2} \approx (10^{-3}$ to
       $10^{-1})~\mbox{m}^{2}~\mbox{s}^{-1}$ with slope $|{\bm S}|
       =10^{-3}$ to $10^{-2}$.
...
\end{equation}
 ...
 \item[$\bullet$] Regularization is required near boundaries and
   regions of steep neutral slopes: \textcite{Cox1987},
   \textcite{GKW91}, \textcite{DanaMcWilliams1995},
   \textcite{ferrari_mcwilliams2008},
   \textcite{dana_ferrari_mcwilliams2008}.
...
\caption{Secondary overturning circulation, ${\bm \Psi}$, arising from
  ...
  thus reducing the available potential energy.  The restratification
  retains density within their layers (``adiabatic'' in ocean
  jargon).}
\end{figure}
...
\item[$\star$] \textcite{GriffiesSkew}: combining the \cite{GM95} scheme 
  + small angle neutral diffusion 
  \begin{multline}
...
\item[$\star$] \textcite{DukSmith1997} suggested that
  ...
  \begin{itemize}
     ...
     \item[$\bullet$] Horizontal tracer fluxes reduce to downgradient
       ...
       \item[$\bullet$] Greatly simplified the model implementation.
   \end{itemize} 
...
Research into mesoscale eddy parameterizations remains an active,
challenging, and exciting area.  Here are a few areas that were either
briefly mentioned or not mentioned at all. Warning: this is an
incomplete list!
...
\begin{itemize}
 ...
 \item[$\star$] Developing subgrid scale parameterizations requires a
  ...
  \begin{itemize}
  \scriptsize 
  \item[$\bullet$] No one approach is sufficient nor more important.
