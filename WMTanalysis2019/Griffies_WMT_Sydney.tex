...
\begin{align}
 ...
 \qquad {\bm u}^{\dagger}  = {\bm u} + {\bm u}^{\mbox{\tiny bolus}} 
\label{eq:gvc-mass-tracer-equation}
\end{align}
...
\begin{center}
{\includegraphics[width=0.8\textwidth]{./figs/ALE_schematic.pdf}}
\end{center}
...
\item {\sc z-coordinate example:} Define $h^{*}$ according to fixed
  z-levels.  Remapping moves the state onto the fixed $z-$grid:
  operationally same as vertical advection.
...
\item[$\star$] Noise term is not physical.  Hence, if it is large it
  can corrupt the physical integrity of the simulation.  
...
\begin{frame}
  \frametitle{Ambiguity and noise in the binning process}
...
\item What bin size should we use?  
   \begin{itemize} \footnotesize 
   \item[$\star$] Bin size can have a nontrivial impact on the results.
\end{itemize}
...
\item Even so, there is no obvious sanity check on WMT
     ...
     \item[$\star$] We are aiming for quantitatively robust
       ...
       the numbers.  That is a problem. 
    \end{itemize}

  \item Are we ready for a detailed best-practices for WMT analysis,
    at least to help minimize obvious mistakes?
...
\begin{frame}
  \frametitle{A final solution for best globally defined buoyancy?}
...
\item Aspirations for any new-and-improved $\gamma$ field.


\begin{itemize}  \footnotesize 
\item[$\star$] An equation for $\gamma$ to allow online binning.

\item[$\star$] An expression for $\gamma$ that is general enough to
  allow for arbitrary model state; not limited to an observational
  dataset.

\item[$\star$] Dispense with the dreaded
  $b-$factor in the following relation from
...
\item Jeff Stanley's ongoing work with Trevor to generalize the
